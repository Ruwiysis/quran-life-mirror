...
async def create_bookmark(data: BookmarkData, authorization: Optional[str] = Header(None)):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            errors.append(f"QF: {r.status_code}")
    except Exception as e:
        errors.append(f"QF error: {e}")
    
    # Fallback to local
    try:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("INSERT OR IGNORE INTO bookmarks (verse_key, created_at) VALUES (?, ?)",
            (data.verse_key, datetime.now().isoformat()))
        conn.commit()
        bookmark_id = cursor.lastrowid
        conn.close()
        return {"id": str(bookmark_id), "verse_key": data.verse_key, "created_at": datetime.now().isoformat()}
    except Exception as e:
        errors.append(f"Local error: {e}")
    
    raise HTTPException(status_code=500, detail=f"Bookmark failed: {errors}")
...
async def get_bookmarks(authorization: Optional[str] = Header(None)):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            print(f"QF Get bookmarks: {r.status_code}")
            if r.status_code == 200:
                data = r.json()
                bookmarks = data if isinstance(data, list) else data.get("bookmarks", data.get("data", []))
                return [{"id": str(b.get("id", "")), "verse_key": b.get("verse_key", ""), "created_at": b.get("created_at", "")} for b in bookmarks]
    except Exception as e:
    ...
    try:
        ...
        conn.close()
        return [{"id": str(r[0]), "verse_key": r[1], "created_at": r[2]} for r in rows]
    except Exception as e:
...
async def delete_bookmark(bookmark_id: str, authorization: Optional[str] = Header(None)):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            r = await client.delete(f"{QF_API_BASE}/auth/api/v1/bookmarks/{bookmark_id}", headers=qf_headers(token))
            if r.status_code in [200, 204]:
                return {"message": "Deleted"}
    except Exception as e:
        print(f"QF delete error: {e}")
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.execute("DELETE FROM bookmarks WHERE id = ? OR verse_key = ?", (bookmark_id, bookmark_id))
        conn.commit()
        conn.close()
...
async def create_note(data: NoteData, authorization: Optional[str] = Header(None)):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if r.status_code in [200, 201]:
                result = r.json()
                return {"id": str(result.get("id", "")), "verse_key": data.verse_key, "note_text": data.note_text, "created_at": result.get("created_at", "")}
    except Exception as e:
        print(f"Note error: {e}")
    return {"id": "local", "verse_key": data.verse_key, "note_text": data.note_text, "created_at": datetime.now().isoformat()}
...
async def get_notes(authorization: Optional[str] = Header(None)):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            r = await client.get(f"{QF_API_BASE}/auth/api/v1/notes", headers=qf_headers(token))
            if r.status_code == 200:
                data = r.json()
...
async def get_streaks(authorization: Optional[str] = Header(None)):
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            r = await client.get(f"{QF_API_BASE}/auth/api/v1/streaks", headers=qf_headers(token))
            if r.status_code == 200:
                data = r.json()
                return StreakInfo(
