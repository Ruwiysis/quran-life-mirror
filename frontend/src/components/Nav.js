...
const T = {
  en: { reflect: 'Reflect', journal: 'Journal', toggle: 'العربية' },
  ar: { reflect: 'تأمّل', journal: 'المذكّرة', toggle: 'English' },
};
...
export default function Nav() {
  ...
  return (
    <nav style={{
      ...
      <Link to="/" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.3rem',fontWeight:400,color:'#c9a84c',textDecoration:'none',letterSpacing:'0.05em'}}>
        {lang === 'ar' ? 'مرآة القرآن ✦' : 'مرآة ✦ Mirror'}
      </Link>
      <div style={{display:'flex',gap:'24px',alignItems:'center'}}>
        {[['/',t.reflect],['/journal',t.journal]].map(([path,label])=>(
          <Link key={path} to={path} style={{
            fontFamily:"'DM Sans',sans-serif",fontSize:'0.85rem',fontWeight:400,
